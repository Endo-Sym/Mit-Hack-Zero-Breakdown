...
- Optional LSTM sequence model Sequence modeling (LSTM) on top-k features
- Alerts generation (threshold + anomaly)
- Metrics: classification_report, ROC-AUC, PR-AUC, lead time
- SHAP explainability
...
def pivot_and_features(df, roll_windows=[3,5,10], compute_fft=False, fft_window=32):
    ...
    if not numeric_cols:
        warn("No numeric columns after constant removal.")
...
def train_stacked_ensemble(df_feat, feature_cols, target_col='FailureLabel', groups=None, n_splits=5, random_state=42):
    ...
    for fold,(train_idx,val_idx) in enumerate(gkf.split(X,y,groups),1):
        ...
        # Correct SMOTE + scaling sequence
        if SMOTE:
        ...
        # XGB
        if xgb:
            clf_xgb = xgb.XGBClassifier(n_estimators=150, max_depth=6, learning_rate=0.08, use_label_encoder=False, eval_metric='logloss', n_jobs=-1, random_state=random_state)
        ...
        # LGB
        if lgb:
            clf_lgb = lgb.LGBMClassifier(n_estimators=150, max_depth=6, learning_rate=0.08, n_jobs=-1, random_state=random_state)
...
# =========================
# 7️) Metrics & Lead Time
# =========================
def compute_metrics(df_feat, pred_col='stack_pred', target_col='FailureLabel'):
    y_true = df_feat[target_col]
    y_pred = df_feat[pred_col] > 0.5
    report = classification_report(y_true,y_pred)
    roc = roc_auc_score(y_true, df_feat[pred_col])
    precision, recall, _ = precision_recall_curve(y_true, df_feat[pred_col])
    pr_auc = auc(recall, precision)
    return report, roc, pr_auc
...
def run_pipeline(sensor_csv="feedmill_clean_long.csv", breakdown_csv="breakdown_log_detailed.csv", out_dir="artifacts"):
    ...
    df = hybrid_labeling(df, breakdown_df)
    df_feat = pivot_and_features(df)
    ...
    # Metrics & Lead Time
    report, roc, pr_auc = compute_metrics(df_feat)
    ...
    return df_feat, alerts_df, report, roc, pr_auc, lead_times
